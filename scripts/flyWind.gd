...
var player = null
var go_to_plr = false
@onready var bullet = preload("res://scenes_for_scenes/ball_boom.tscn")
...
func _physics_process(delta):
	...
	if canFire: # and not get_parent().get_node_or_null("BALL-boom")
		...
		cd.start(rng.randf_range(3.0, 10.0))
		bullet_instance.position = position
		bullet_instance.position.y += 100
	...
	if go_to_plr:
		...
		if position.x >= player.position.x:
			...
			$AnimatedSprite2D.flip_h = false #Отзеркаливаем горизонтально
		else: #Если позиция x меньше конца пути, то
			direction = 1 #Меняем направление назад
			$CollisionShape2D.position.x = 35
			
			$hitBox/CollisionShape2D.position.x = 35
			$AnimatedSprite2D.flip_h = true #Отзеркаливаем горизонтально обратно
	else:
		SPEED = 3
		if position.x >= end_x:
			direction = -1 #Меняем направление назад
